...
{
    // https://github.com/dotnet/aspnetcore/blob/master/src/Mvc/Mvc.Testing/src/Handlers/RedirectHandler.cs
    // This redirect handler does not strip the Authorization header on redirects
    public class RedirectMessageHandler : DelegatingHandler
    {
        internal const int DefaultMaxRedirects = 7;

        /// <summary>
        /// Creates a new instance of <see cref="RedirectMessageHandler"/>.
        /// </summary>
        ...
        public RedirectMessageHandler(int maxRedirects)
        {
            if (maxRedirects <= 0)
            {
